...
if st.session_state["user"] is None:
    ...
    if _cookie_brut:
        try:
            ...
            if jeton_valide:
                issued_at_dt = datetime.datetime.fromisoformat(
                    remembered_issued_at
                )
                session_expiree = (
...
def render_view(module, supabase_client):
    if module is None:
        st.error("⚠️ Ce module n'est pas disponible ou n'a pas pu être chargé.")
        return
    if not hasattr(module, "show"):
        st.error(
            f"⚠️ Le module `{getattr(module, '__name__', 'sélectionné')}` ne contient pas la fonction `show()`."
        )
        return
    try:
        module.show(
        ...
        try:
            ...
            try:
                module.show(supabase_client, user=st.session_state.get("user"))
            except TypeError:
        ...
        if os.path.exists(image_path):
            try:
                ...
                st.image(
                    img,
                    caption=(
                        "Al Boraq - Ligne à Grande Vitesse - Projet LGV CASA SUD"
                    ),
                    use_container_width=True,
                )
            except Exception as e:
                ...
                if user_to_delete == current_username:
                    ...
                    if st.button(
                        ...
                        if success:
                            st.session_state["users_db"] = load_users()
                            st.success(
                                f"🗑️ Utilisateur **{user_to_delete}** supprimé"
                                " définitivement."
                            )
                            st.rerun()
                        else:
