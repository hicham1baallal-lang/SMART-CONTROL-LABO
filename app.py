...
def _normaliser_projets_utilisateur(projets):
  """Accepte les anciens enregistrements texte et les listes Supabase."""
  if not projets:
    return []
  if isinstance(projets, list):
    return [str(p).strip() for p in projets if str(p).strip()]
  return [p.strip() for p in str(projets).split(",") if p.strip()]
      ...
      if res.data:
        for row in res.data:
          ...
          users[row["username"]] = {
              ...
              "projets_autorises": _normaliser_projets_utilisateur(projets_bruts),
          }
    except Exception:
      pass
  return users
          ...
          if not new_username:
            ...
            if success:
              ...
              st.error(
                  ...
                  " désactivé sur la table users dans Supabase.*"
              )
