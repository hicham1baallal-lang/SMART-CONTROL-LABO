...
def generate_pdf(header_info, data_dict, type_mat):
    ...
    pdf.cell(0, 8, f"PROCES VERBAL - GRANULO & IDENTIFICATION ({type_mat.upper()})", 0, 1, "C")
    pdf.ln(5)
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(190, 8, " II - Synthèse Granulométrique & Paramètres (NM 00.8.082 / LPEE)", 1, 1, "L", fill=True)
    pdf.set_font("Helvetica", "", 9)
...
def show(supabase_client):
    ...
    f_st.title("🔬 Identification & Granulométrie Matériaux")
    ...
    sub_material_choices = [
        "Remblai ordinaire",
        "GNF 0/40",
        "GNA 0/31.5",
        "Couche de forme / sous couche 0/31.5",
    ...
    with tab_saisir:
        ...
        f_st.markdown(f"### 📄 Feuille d'Essai type LPEE — {type_mat}")
        
        col_e1, col_e2, col_e3, col_e4 = f_st.columns(4)
        with col_e1:
            ref_ech = f_st.text_input("Référence Échantillon", value="ECH-MAT-0247", disabled=not user_can_edit)
        with col_e2:
        ...
        is_conf = (pass_80um_val <= 35.0) and (ip < 25)
        obs = f"Conforme {type_mat} (Passant 80µm={pass_80um_val:.1f}%)" if is_conf else f"Non Conforme / Hors fuseau ({type_mat})"

        f_st.info(f"Observation automatique : **{obs}** | Dmax: **{dmax_detected} mm** | Passant 2mm: **{pass_2mm_val:.1f}%** | Écart référence: **{val_ecart:.2f}%**")
        ...
        with col_d1:
            f_st.download_button("📄 Télécharger PV (PDF)", data=pdf_bytes, file_name=f"PV_Identification_{num_rapport.replace('/','_')}.pdf", mime="application/pdf", use_container_width=True)
        with col_d2:
            if f_st.button("💾 Enregistrer dans Supabase", type="primary", use_container_width=True, disabled=not user_can_edit):
                payload_record = {
                    "num_rapport": num_rapport,
                    "type_materiau": type_mat,
                    "lieu": lieu,
                    "pk": pk,
    ...
    with tab_synth:
        ...
        if data_to_use:
            ...
            sub_count = len(df_s)
            m3.metric("Enregistrements", sub_count)
